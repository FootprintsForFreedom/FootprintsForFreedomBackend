...
final class WaypointApiSearchTests: AppTestCase, WaypointTest, TagTest {
    func testSuccessfulSearchWaypointReturnsWhenTextInTitle() async throws {
        let waypoint = try await createNewWaypoint(title: "Ein besonderer Titel", detailText: "Anderer Text", verified: true)
    ...
    func testSuccessfulSearchWaypointOnlyReturnsVerifiedWaypoints() async throws {
        let waypoint = try await createNewWaypoint(title: "Ein besonderer Titel", detailText: "Anderer Text", verified: false)
        try await waypoint.detail.$language.load(on: app.db)
    ...
    func testSuccessfulSearchWaypointReturnsWhenTextInTagTitle() async throws {
        ...
        let tag = try await createNewTag(title: "Ein besonderer Titel", keywords: ["Anders"], verified: true, languageId: language.requireID())
    ...
    func testSuccessfulSearchOnlySearchesTagsInSpecifiedLangauge() async throws {
        let tag = try await createNewTag(title: "Ein besonderer Titel", keywords: ["Anders"], verified: true)
    ...
    func testSuccessfulSearchWaypointOnlyReturnsDetailsForSpecifiedLanguage() async throws {
        ...
        let waypoint = try await createNewWaypoint(title: "Ein besonderer Titel", detailText: "Anderer Text", verified: true, languageId: language.requireID())
        try await waypoint.detail.$language.load(on: app.db)
